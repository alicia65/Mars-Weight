...
namespace Mars_Weight
{
    public partial class Form1 : Form
    {
        ...
        private void btnCalculate_Click(object sender, EventArgs e)
        {
            ...
            if (!ValidatePositiveDouble(txtWeightOnEarth.Text, out double earthWeight, out string weightErrorMessage))
            {
                MessageBox.Show(weightErrorMessage, "Earth Weight Error");
                txtWeightOnEarth.Focus(); //Move focus to this TextBox for user to correct error
                return; // stop processing this event
            }

            double conversionFactor = 0.377;
            double marsWeight = earthWeight * conversionFactor;
            txtWeightMars.Text = String.Format("{0} weight {1} on Mars", objectName, marsWeight);
        }
